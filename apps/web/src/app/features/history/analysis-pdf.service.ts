...
export class AnalysisPdfService {
  ...
  private drawFundamentalsDisclaimer(doc: jsPDF, y: number): void {
    ...
    const lines = wrap(
      doc,
      'Every reading in this document is generated by the model from the fundamentals data alone. TradeSathi does not connect to a broker, hold positions, or track what happens after an analysis.',
      CONTENT_W,
    );
